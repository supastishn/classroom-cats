...
const onFrame = () => {
    ...
    clearTimeout(window.catSleep)

	if (volume >= document.getElementById("myRange").value * 0.008) {
	clearTimeout(window.catCome)
	document.getElementById("run").style.display = "block"
	document.getElementById("awake").style.display = "none"
        document.getElementById("asleep").style.display = "none"
        console.log("oops the cat is RUNNING WHAT HAVE YOU DONE")
       window.catCome = setTimeout(() => {
